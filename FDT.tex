...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[C]{\small A Conditional Fluctuation--Dissipation Identity for Market Impact}
\fancyfoot[C]{\thepage}
...
\newcommand{\ImPart}{\operatorname{Im}_{\!+}}
\newcommand{\dd}{\,\mathrm{d}}
\newcommand{\REV}{}

\title{A Conditional Fluctuation--Dissipation Identity for Market Impact:\\
Diagnostics, Reconstruction, and Order-Book Evidence}
...
\begin{abstract}
\REV
Market impact and price volatility are usually studied as separate objects. This paper asks whether they can be compared in a single frequency-domain framework. The core idea is a conditional fluctuation--dissipation relation: if price pressure is carried by hidden liquidity modes that decay at characteristic speeds, then the dissipative part of the market's impact response must line up with its price fluctuation spectrum through
\[
  \ImPart \Ghat(i\omega)=\frac{\omega}{2\Theta}S_P(\omega),
\]
where $\Ghat$ is the impact kernel in frequency space, $S_P$ is the price spectrum, and $\Theta$ is a single market thermal index. The relation is used as a testable benchmark rather than a universal claim: when the two sides match, the market is consistent with the model; when they separate, the model is rejected on that frequency band.

The paper develops the latent modal equilibrium model from which this identity follows, constructs a practical spectral test and a null-enforcing bootstrap, and builds an inverse problem for recovering the hidden relaxation spectrum. Monte Carlo simulations show controlled size and increasing power as sample size grows. An empirical illustration using public order-book data from June 2012 identifies one rejecting intraday window in a three-stock cross-section, though the evidence is too narrow for broader market-structure claims. A separate May 2026 public-bar proxy screen for five U.S. and five Indian stocks is reported as a recency check: the signed-square-root-volume proxy retains 28 windows with 11 rejections, while raw signed-volume retains none. These results are preliminary, but they demonstrate that the diagnostic can separate ordinary intraday variation from a specific breakdown in the impact-volatility relation.
\end{abstract}

\noindent\textbf{Keywords:} market impact; fluctuation--dissipation; spectral analysis; order-book microstructure; Stieltjes kernel; latent modal equilibrium
...
Symbol & Meaning \\
\midrule
$\Ghat(i\omega)$ & Fourier--Laplace transform of the impact kernel $G(t)$ \\
$\ImPart \Ghat(i\omega)$ & Positive dissipative (imaginary) part of $\Ghat$ \\
$S_P(\omega)$ & Price power spectral density \\
$\Theta$ & Market thermal index (scalar) \\
$\widehat\Theta$ & Median-of-ratios estimate of $\Theta$ \\
$\Delta(\omega)$ & FDT deviation functional: $\ImPart\Ghat(i\omega)-\frac{\omega}{2\Theta}S_P(\omega)$ \\
$\widehat\Delta(\omega_j)$ & Estimated deviation at Fourier frequency $\omega_j$ \\
$\widehat G_\eta(\omega_j)$ & Ridge-regularized transfer estimator at $\omega_j$ \\
$\eta_n$ & Ridge parameter for spectral inversion \\
$M(\dd\rho)$ & Stieltjes relaxation measure on $(0,\infty)$ \\
$r_t$ & Log return over interval $t$ \\
$q_t$ & Signed order imbalance over interval $t$ \\
$\cT_n$ & Bootstrap test statistic \\
$m$ & Smoothing-window half-width (Fourier frequencies) \\
\bottomrule
\end{tabular}
\label{tab:notation}
\end{table}
...
The paper does not claim to derive this identity from a fully general strategic equilibrium model. Instead, it formulates a latent modal equilibrium model under which the identity must hold and then asks whether that restriction is empirically useful and empirically falsifiable. The theory is deliberately more ambitious than the empirical evidence can fully support; the gap between the two is the central limitation of the study and is addressed explicitly in the closing sections.
...
\begin{remark}[Proof-of-concept framing]
The paper is intended as a theoretical and empirical proof-of-concept rather than a definitive market-wide validation. The empirical evidence is narrow and preliminary, and the framework's practical value will depend on future multi-week, multi-market studies using licensed TAQ data.
\end{remark}
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item An empirical layer consisting of Monte Carlo evidence, rolling synthetic validation, real-data LOBSTER evidence with a same-date three-name cross-section, and a clearly separated May 2026 public-bar proxy screen.
\end{enumerate}
...
The paper proceeds in three steps. \Cref{sec:equilibrium,sec:fdt} introduce the latent modal equilibrium class and the conditional FDT identity it implies. \Cref{sec:testing,sec:inverse} convert that identity into an estimable diagnostic and, when the null is not rejected, an inverse problem for the hidden relaxation spectrum. \Cref{sec:monte-carlo,sec:rolling,sec:lobster-pilot,sec:roadmap} then show what the diagnostic looks like in synthetic data, in a focused real-data order-book study, and in a separate current public-bar proxy screen.
...
\end{equation}
Throughout the paper $\ImPart \Ghat(i\omega)$ denotes the positive dissipative component, i.e. $-\operatorname{Im}\Ghat(i\omega)$ under the convention in \cref{eq:laplace-transform}. This sign convention matches the nonnegative Stieltjes representation in \cref{eq:imaginary-part} and the empirical estimator used below.
...
\begin{remark}[Model scope and strong assumptions]
The framework rests on several strong restrictions that delimit its scope. First, the kernel $G$ is restricted to the Stieltjes class, which is economically motivated by no-manipulation admissibility but is not universal --- nonlinear impact, asymmetric response to buys versus sells, and regime-switching kernels are excluded by construction. Second, the price model in \cref{eq:price-model} is linear, which rules out nonlinear microstructure effects such as discontinuous order-book replenishment and price-contingent order flow. Third, the public-information martingale $L(t)$ is assumed orthogonal to order flow, a strong independence condition that may fail in practice when information events trigger both order arrivals and price revisions. The empirical tests therefore evaluate a structured null hypothesis --- the data are consistent with this specific latent modal equilibrium class --- rather than a universal property of all markets.
\end{remark}
...
\begin{proof}[Proof of \cref{prop:theta-rate}]
\REV
...
\begin{remark}[Practical utility of the inverse reconstruction]
The spectral inverse problem is severely ill-posed (\cref{prop:severe-illposed}), with singular values decaying exponentially and a convergence rate of only $O(|\log\delta|^{-1})$ under logarithmic source conditions. In practical terms, even small noise in the estimated price spectrum translates into large uncertainties in the recovered relaxation measure. The LASSO discretization provides a tractable computational surrogate, but its support-localization guarantees depend on a log-scale separation condition (C3) that may not hold for real markets where relaxation times are continuously distributed. These caveats should be weighed when assessing the operational value of the kernel reconstruction: the main practical payoff of the framework lies in the diagnostic test and the market-state map, not in the inverse problem.
\end{remark}
...
$2^{14}$ & 500 & 16 & $0.034\,(0.008)$ & $0.328\,(0.021)$ \\
$2^{16}$ & 500 & 32 & $0.036\,(0.008)$ & $0.680\,(0.021)$ \\
\bottomrule
\end{tabular}
\vspace{0.4em}
...
To demonstrate that the pipeline can be executed on genuine stock-market data, we use public LOBSTER samples for June 21, 2012. The main single-name illustration is AAPL, and we also run the same protocol on same-date samples for GOOG and AMZN to obtain a minimal liquid-stock cross-section under a common market environment. The samples contain the full NASDAQ message stream and the synchronized top-of-book state over the continuous trading day from 9:30 to 16:00. We aggregate the data to one-minute bins. Midquote returns are computed from the last best bid and best ask observed in each bin, and signed order flow is computed from executed volume only, using LOBSTER event types 4 and 5 with the LOBSTER direction field. This is not yet a multi-week TAQ panel, but it is genuine message-data evidence with exchange-derived execution signs rather than a price-based proxy.\footnote{The raw public folder also contains MSFT and INTC samples. Under the retained-band positivity and nondegeneracy rule used for the reported estimator, those two names retained no valid open/midday/close window; no rejection or non-rejection claim is made for them.}
...
\midrule
Open   & 4.094 & 0.216 & 1.000 & No \\
Midday & 1.173 & 1.104 & 0.003 & Yes \\
Close  & 2.236 & 0.686 & 0.358 & No \\
\bottomrule
\end{tabular}
\vspace{0.4em}
...
To test whether the AAPL intraday pattern is idiosyncratic, we apply the same
open/midday/close protocol to public same-date LOBSTER samples for GOOG and
AMZN. Table~\ref{tab:lobster-cross-section} summarizes the resulting
diagnostics.
...
\midrule
AAPL & Open   & 4.094 & 0.216 & No \\
AAPL & Midday & 1.173 & 1.104 & Yes \\
AAPL & Close  & 2.236 & 0.686 & No \\
GOOG & Open   & 0.218 & 0.294 & No \\
GOOG & Midday & 0.455 & 0.557 & No \\
GOOG & Close  & 1.807 & 0.583 & No \\
AMZN & Open   & 3.849 & 0.322 & No \\
AMZN & Midday & 2.688 & 0.612 & No \\
AMZN & Close  & 0.819 & 0.519 & No \\
\bottomrule
\end{tabular}
\vspace{0.4em}
...
\subsection{Recent Public-Bar Proxy Screen}\label{sec:recent-proxy}
\REV
The June 2012 LOBSTER samples remain the main real-data evidence because they contain signed execution information. To address recency, the replication package also includes a current public-data screen using 5-minute OHLCV bars. The screen covers AAPL, MSFT, JPM, XOM, and NVDA over May 15--29, 2026, and RELIANCE.NS, HDFCBANK.NS, INFY.NS, TCS.NS, and ICICIBANK.NS over May 18--29, 2026. Each trading day is split into open, midday, and close thirds, and two price-based order-flow proxies are checked: signed volume and signed square-root volume.

This screen should be read as a recency check, not as order-book evidence. The raw signed-volume proxy does not pass the retained-frequency positivity filter in any of the 30 attempted windows. The signed-square-root-volume proxy is better behaved: it produces 28 usable windows, with 4 rejections in the U.S. sample and 7 rejections in the India sample. The result shows that the pipeline can be run on current public stock-market data, but it also shows why the paper keeps the main evidentiary weight on signed message data.

\begin{table}[H]
\centering
\caption{Recent public OHLCV proxy screen (5-minute bars, May 2026)}
\small
\begin{tabular}{llccccc}
\toprule
Market & Sample dates & Raw valid & Sqrt valid & Sqrt rejects & Avg. $\widehat{\Theta}$ & Avg. ratio \\
\midrule
U.S. & May 15--29, 2026 & 0/15 & 14/15 & 4/14 & 1.792 & 0.577 \\
India & May 18--29, 2026 & 0/15 & 14/15 & 7/14 & 1.478 & 0.726 \\
\bottomrule
\end{tabular}
\vspace{0.4em}

\parbox{0.94\textwidth}{\footnotesize ``Raw valid'' reports retained windows for the signed-volume proxy. ``Sqrt valid'' and ``Sqrt rejects'' report retained windows and bootstrap rejections for the signed-square-root-volume proxy. The table is based on public 5-minute bars rather than signed executions, so it is reported as a proxy-tier screen only.}
\label{tab:recent-proxy}
\end{table}
...
\section{Interpretation and Scope}\label{sec:roadmap}
...
\REV
The real-data evidence in this paper is severely limited and should be read as a focused order-book study, not as a market-wide validation exercise. The LOBSTER cross-section is a single-date $3\times3$ table in which only one cell (AAPL midday) rejects. A single rejection in nine cells is not statistically convincing for broader claims about market structure, and without a multi-week panel or cross-sector analysis it remains unclear whether the AAPL midday signal reflects a genuine economic breakdown or an estimation artifact specific to that stock, window, and day.

The May 2026 public-bar screen improves recency but not identification quality. Only 28 windows survived the retained-band filter, with 11 rejections distributed unevenly across two markets. This is too sparse for current-market validation and relies on price-based signed-flow proxies rather than exchange-signed executions. Broader panel claims are therefore not inferred from either sample.

\subsection{Proxy-tier extensions}
\REV
The repository includes an exploratory OHLCV proxy-screen script for deployments where signed message data are not available. \Cref{sec:recent-proxy} reports the resulting May 2026 screen because it is useful for recency and operational transparency. The reason it remains proxy-tier is simple: bar data do not contain true execution direction, so any signed-flow proxy would require its own market-specific calibration before supporting stress-monitoring claims. This is a meaningful practical constraint, because the price-based proxies tested here (signed volume and signed square-root volume) largely fail to retain valid windows under the estimator's filter. Practical deployment of the diagnostic outside of research contexts would therefore depend on access to licensed TAQ feeds, which most practitioners lack.

The publishable order-book evidence is therefore still the public LOBSTER message-data study.

\subsection{Data and Code Availability}
Simulation code, plotting scripts, figure data, and the real-data analysis scripts used for the reported figures are collected in the public replication repository at \url{https://github.com/Atharva12081/fdt-market-impact}. The order-book study is based on public LOBSTER samples for AAPL, GOOG, and AMZN on June 21, 2012, with MSFT and INTC retained in the raw data folder but excluded by the estimator's retained-band filter. The recent proxy screen is generated by \texttt{scripts/run\_recent\_market\_proxy.py}, which downloads public 5-minute OHLCV bars and writes \texttt{figures/data/recent\_market\_proxy\_results.csv} and \texttt{figures/data/recent\_market\_proxy\_summary.txt}. Any future extension using licensed TAQ data remains subject to the corresponding vendor restrictions. The author thanks colleagues and seminar participants for helpful discussions. No external funding was received, and the author declares no competing interests.

\section{Conclusion}
\REV
This study develops a conditional fluctuation--dissipation framework for transient market impact that is both structurally coherent and empirically testable. On the theory side, it analyzes the consequences of an explicit latent modal equilibrium model, links the resulting identity to passivity and no manipulation, and gives a linear-Gaussian foundation for the modal scaling. On the statistical side, the paper provides a concrete test, a structured inverse problem, controlled Monte Carlo evidence, and empirical illustrations.

The empirical contribution is narrow by design. The LOBSTER cross-section is a single-date $3\times3$ table with one rejecting cell, which limits any broader market-structure claim. The May 2026 proxy screen is weakened by its reliance on price-based signed-flow proxies. The theory is more ambitious than the evidence can fully support, and the gap between the latent modal equilibrium class and the narrow empirical test is the main limitation of the paper as it stands.

Within these bounds, the strongest payoff is the separation of the thermal index and the deviation functional into two distinct coordinates. In the signed order-book data studied here, that separation identifies one breakdown cell --- AAPL at midday --- while the same-date GOOG and AMZN windows remain non-rejecting. Whether this reflects a genuine market structure break or an estimation artifact cannot be settled with the current single-day panel. Extending the analysis to multi-week, multi-sector TAQ panels is the natural next step.

\appendix

\section{Empirical Construction Details}
\REV
The LOBSTER study uses message-file event types $4$ and $5$ only, corresponding respectively to visible and hidden executions. The trading day is partitioned into consecutive one-minute bins on $[9{:}30,16{:}00)$, and each message is assigned to the unique bin containing its exchange timestamp. The midquote in a bin is the last observed top-of-book midpoint in that bin, namely $(\text{best ask}+\text{best bid})/2$, with carry-forward of the most recent midpoint if no new quote arrives before the bin closes. Signed order flow is the sum of executed shares in the bin with the sign taken directly from the LOBSTER direction field: $+1$ denotes execution of a buy limit order and $-1$ denotes execution of a sell limit order. Returns are one-minute log differences of the carried-forward midquote sequence. The ridge-regularized transfer estimator uses the local rule $\eta_n(\omega_j)=0.01\,\widehat S_Q(\omega_j)$ on the retained band.

\REV
The May 2026 proxy screen uses public 5-minute OHLCV bars. Timestamps are converted to the local exchange timezone, each trading day is split into three equal open/midday/close blocks, and returns are log differences of bar closes. The signed-volume proxy sets $Q_t=\operatorname{sign}(r_t)V_t$, while the signed-square-root-volume proxy sets $Q_t=\operatorname{sign}(r_t)\sqrt{V_t}$, where $V_t$ is bar volume. These are price-based proxies, not signed executions. For that reason the proxy screen is reported separately from the LOBSTER evidence and is not used to calibrate the order-book conclusions.
